...
def validate_tex(path: Path) -> dict[str, Any]:
    ...
    if min(title_pos, abstract_start, abstract_end, keyword_pos, first_section) < 0:
        errors.append(issue("missing_abstract_front_page", "title, Chinese abstract, keywords and a body section are required"))
    ...
    for label in dict.fromkeys(semantic_equations + equation_labels):
    ...
    table_blocks = re.findall(r"\\begin\{table\}.*?\\end\{table\}", clean, flags=re.S)
    longtable_blocks = re.findall(r"\\begin\{longtable\}.*?\\end\{longtable\}", clean, flags=re.S)
    for index, block in enumerate(table_blocks + longtable_blocks, start=1):
    ...
    result = {
        ...
        "summary": {
            ...
            "table_like_count": len(table_blocks) + len(longtable_blocks),
        },
    }
    return result
